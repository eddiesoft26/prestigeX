...
import toast, { Toaster } from "react-hot-toast"; // Recommended for feedback
...
const CreateWallet = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post("/admin/create-wallet", formData);
      toast.success(`${formData.coin} ${res.data.message}`);
      setFormData({ coin: "", address: "", network: "ERC20" });
    } catch (err) {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Configure global receiving addresses for investor deposits.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <form
          ...
          className="bg-[#1A1D21] border border-white/5 p-8 rounded-[2rem] space-y-6"
        >
          <div className="space-y-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Bitcoin or ETH"
                ...
                value={formData.coin}
                onChange={(e) =>
                  setFormData({ ...formData, coin: e.target.value.toUpperCase() })
                }
                required
              />
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-700 text-black font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            {loading ? (
              "Syncing..."
            ) : (
              <>
                <HiOutlineSave size={20} /> Deploy Wallet
              </>
            )}
          </button>
        </form>

        {/* PREVIEW CARD  YET TO BE IMPLEMENTED*/}
        <div className="bg-gradient-to-br from-amber-500/20 to-transparent border border-amber-500/10 p-8 rounded-[2rem] flex flex-col justify-between relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <p className="text-[10px] font-black text-amber-500/60 uppercase tracking-tighter">
                  Live Preview
                </p>
                <h4 className="text-white font-bold uppercase">
                  {formData.coin || "Asset Name"}
                </h4>
              </div>
            </div>
            ...
            <div className="space-y-4 relative z-10">
              <div>
                ...
                <p className="text-sm font-mono text-slate-300 break-all bg-black/40 p-3 rounded-lg mt-1 border border-white/5">
                  {formData.address || "bc1qxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}
                </p>
              </div>
          ...
          <div className="mt-8 pt-8 border-t border-white/5">
            <p className="text-[10px] text-slate-500 italic">
              * Once deployed, this address will be visible to all users in the
              Deposit section.
            </p>
          </div>
        </div>
